...
class ShowTotals extends Renderer {
  ...
  async render() {
    ...
    this.addStatItem(statsGrid, `${this.caloriesPerOunce().toFixed(0)}`, "Cal/oz")
    this.addStatItem(statsGrid, `${(this.totalNetWeight() / 453.592).toFixed(1)} lbs`, "Weight")
  ...
  createProgressBar(label, current, goal, variant = "") {
    ...
    const labelEl = document.createElement("div")
    labelEl.style.cssText = "display: flex; justify-content: space-between; align-items: center; margin-bottom: var(--spacing-1);"
    
    const labelText = document.createElement("span")
    labelText.style.cssText = "font-size: var(--font-size-sm); font-weight: var(--font-weight-medium); color: var(--text-primary);"
    labelText.textContent = label
    
    const percentage = Math.min((current / goal) * 100, 100)
    const percentageText = document.createElement("span")
    percentageText.style.cssText = "font-size: var(--font-size-sm); color: var(--text-tertiary);"
    percentageText.textContent = `${current}/${goal} (${percentage.toFixed(0)}%)`
