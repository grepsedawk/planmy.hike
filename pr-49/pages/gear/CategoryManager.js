...
class CategoryManager {
  ...
  async saveCategory() {
    ...
    try {
      ...
      if (
        window.currentGearPage &&
        typeof window.currentGearPage.loadGear === "function"
      ) {
  ...
  async updateCategory(id) {
    ...
    try {
      // Get the existing category to preserve dateCreated
      ...
      const categoryData = {
        ...
        description: descInput.value.trim(),
        color: colorInput.value,
        dateCreated: existingCategory.dateCreated, // Preserve original creation date
      }
  ...
  async deleteCategory(id) {
    ...
    if (itemCount > 0) {
      const confirmed = confirm(
        `Category "${category.name}" contains ${itemCount} item(s). Deleting it will move those items to "Uncategorized". Continue?`,
      )
      if (!confirmed) return
